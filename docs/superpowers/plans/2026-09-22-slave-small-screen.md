...
**Architecture:** 保持原 50 个 holding registers 的位置，小屏作为 Modbus 主站轮询，分机只响应。新接口明确按寄存器序列化，不直接发送 C 联合体；普通字高字节在前，浮点 IEEE754 高字在前（ABCD），在接线文档给出验证帧。新增通道只读，不抢占联控、不改控制命令和参数。现场已确认只有接触器、没有可控硅，默认第一路给小屏，第二路保留联控。
...
- [x] 现场确认没有可控硅调功器；默认使用 PA9/PA10/PA8 接小屏，禁用原调功器轮询及该路调试文字，PA2/PA3/PA4 保留联控。不能把两个 Modbus 主站并到一个端口。
- [x] 小屏工程目标站号已确认为2，固件固定匹配2；联控拨码地址独立。验证联控地址同为2及不同为4时，读屏不修改联控地址、状态或待处理请求，新小屏接口拒绝旧站号254。
